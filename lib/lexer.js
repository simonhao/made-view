...
Lexer.prototype = {
  ...
  attrs: function(use_return){
    ...
    while(str !== ''){
      ...
      if(key_val.key){
        if(detect.is_string(key_val.val)){
          key_val.val = key_val.val.replace(/^['"]|['"]$/g, '');
          this.assert_interpolate(key_val.val);
        }else{
          is_string = false;
          this.assert_expression(key_val.val);
        }

        tok.attrs.push({
          name: key_val.key,
          val: key_val.val,
          type: is_string ? 'string' : 'expression'
        });
      }
  ...
  fail: function(){
    this.error('Invalid token :' + this.input.substring(0, 20), 'SYNTAX_ERROR');
  },
  next: function(){
    return this.doctype()
      || this.block()
