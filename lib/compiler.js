...
Compiler.prototype = {
  ...
  buffer_attr: function(attr){
    if(utils.is_string(attr)){
      this.buffer_text(attr.replace(/^['"]|['"]$/g, ''), true, true)
    }else{
      this.buffer_code('made_buf.push(made.escape((' + attr + ')));');
    }
  },
  /**
   * 清除文本缓冲区
    ...
    function interpolate(str){
      ...
      if(match){
        if(match[1]){
          ...
          interpolate(rest.substring(range.end + 1));
        }
      }else{
  ...
  interpolate: function(str, escape){
    ...
    if(match){
      ...
      if(match[1]){ // escape
        ...
        this.interpolate(rest.substring(range.end + 1), escape);
      }
    }else{
      this.last_text_buf.push(escape ? runtime.escape(str) : str);
    }
  ...
  visit_tag: function(node){
    ...
    node.attrs.forEach(function(attr){
      if(attr.name === 'class'){
        class_buf.push(attr.val);
      }else if(attr.name === 'id'){
        self.buffer_text(' id="');
        self.buffer_text(self.sid + (self.options.instance ? '-' + self.options.instance : '') + '-');
        self.buffer_attr(attr.val);
        self.buffer_text('"');
      }else{
        self.buffer_text(' ');
  ...
  visit_attr: function(attr){
    ...
    this.buffer_attr(attr.val);
    this.buffer_text('"');
  },
  visit_text: function(node){
    this.buffer_text(node.val, true, true);
  },
  visit_extends: function(node){
    ...
    var js = new Compiler(str, extend({}, this.options, {
      filename: filename,
      instance: this.sid + (this.options.instance ? '-' + this.options.instance : '')
    })).compile();
    ...
    node.options.forEach(function(option){
      options_buf.push(option.name +':(' + self.get_options_val(option.val) + ')');
    });
