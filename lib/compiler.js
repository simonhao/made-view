...
Compiler.prototype = {
  ...
  buffer_attr: function(attr){
    if(attr.type === 'string'){
      this.buffer_text(attr.val.replace(/^['"]|['"]$/g, ''), true, true)
    }else{
  ...
  get_path: function(id){
    ...
    var info = {
      ...
      instance: important_instance ? important_instance : (this.sid + (this.options.instance ? '-' + this.options.instance : '') + (module_instance ? '-' + module_instance : ''))
    };
