...
Compiler.prototype = {
  ...
  detect_path: function(str){
    ...
    if(this.sid) instance.push(this.sid);
    if(this.instance) instance.push(this.instance);
  ...
  interpolate: function(str, inline){
    ...
    if(match){
      ...
      if(match[1]){ // escape
        ...
        var code = (('!' !== match[2] && inline) ? 'made.encode' : '') + '(' + range.src + ')';
  ...
  visit_tag: function(node){
    ...
    property_list.forEach(function(property){
      if(property.name === 'class'){
        ...
        self.buffer_text(' ');
        self.visit_property(property, node.name);
      }
    });
  ...
  visit_property: function(node, tag_name){
    if(node.name in this.transform){
      this.visit_transform_property(node, tag_name);
  ...
  visit_transform_property: function(node, tag_name){
    if(node.val && node.val.type === 'json_string'){
      this.buffer_text(node.name + '="' + this.transform[node.name](node.val.val, tag_name, this.options) + '"');
    }else{
  ...
  visit_json_ident: function(node, inline){
    if(inline){
      this.buffer_code_last('made.encode(' + node.val + ')', inline);
    }else{
  ...
  visit_json_reference_list: function(node, inline){
    if(inline){
      this.buffer_code_last('made.encode(' + node.ident.val, inline)
  ...
  visit_extends: function(node){
    ...
    var compiler = new Compiler(ast, extend({}, this.options, {
      ...
      sid: mid.sid(path_info.filename, this.options),
  ...
  visit_each: function(node){
    this.buffer_code('made.each(' + node.data + ',function(' + node.value + (node.index?','+node.index : '') + '){');
    this.visit_nodes(node.nodes);
    this.buffer_code('})');
  },
