...
Compiler.prototype = {
  ...
  detect_path: function(str){
    ...
    var info = {
      ...
      instance: (important_instance !== undefined) ? important_instance : (this.sid + (this.instance ? '-' + this.instance : '') + (module_instance ? '-' + module_instance : ''))
    };
  ...
  visit_nodes: function(nodes, inline, dot_indent){
    var self = this;

    if(!Array.isArray(nodes)) return;


    if(!dot_indent) this.indent();

    nodes.forEach(function(node){
      self.visit(node, inline);
    });

    if(!dot_indent) this.outdent();
  },
  visit_file: function(file){
    var self = this;

    if(!Array.isArray(file.nodes)) return;
  ...
  visit_custom_tag_extend: function(node){
    ...
    this.visit_extends(extend_node);
  },
  visit_custom_tag_include: function(node){
    ...
    this.visit_include(include_node);
  }
};
